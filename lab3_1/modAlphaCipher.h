...
private:
     ...
     * @details Проверяет, что ключ не пустой и содержит только допустимые символы алфавита.
     * Переводит символы из нижнего в верхний регистр.
     ...
     * @details Проверяет, что открытый текст не пустой, содержит только допустимые символы алфавита.
     * Переводит символы из нижнего в верхний регистр.
     * @param s Входная строка, представляющая открытый текст
     * @throw cipher_error Если открытый текст пустой или содержит недопустимые символы
     */
    std::wstring getValidOpenText(const std::wstring& s);
    /**
     * @brief Валидация зашифрованного текста
     * @details Проверяет, что зашифрованный текст не пустой, содержит только допустимые символы алфавита.
     * @param s Входная строка, представляющая зашифрованный текст
...
public:
 	...
 	* @param cipher_text Шифрованный текст должен быть пустой строкой.
 	* Строчные символы автоматически преобразуются к прописным.
 	* Все не-буквы удаляются
 	* @return Зашифрованная строка
 	* @throw cipher_error, если текст пустой
 ...
 * @details Класс, созданный для обработки ошибок 
 */
